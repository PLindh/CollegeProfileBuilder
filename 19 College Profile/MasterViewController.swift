...
class MasterViewController: UITableViewController {
    ...
    func insertNewObject(_ sender: Any) {
        ...
        alert.addTextField { (textField) in textField.placeholder = "College"
        }
        alert.addTextField { (textField) in
             textField.placeholder = "Location"
        }
        alert.addTextField { (textField) in
            textField.placeholder = "Number Students"
        ...
        let insertAction = UIAlertAction(title: "Add", style: .default) {
            ...
            let numStudentsTextField = alert.textFields![2] as UITextField
            guard let image = UIImage(named: nameTextField.text!) else {
                print("missing \(nameTextField.text!) image")
                return
            }
            if let numStudents = Int(numStudentsTextField.text!) {
